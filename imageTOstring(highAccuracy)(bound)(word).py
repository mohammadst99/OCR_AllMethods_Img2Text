...
def text(img):
    ...
    stringDATA=pytesseract.image_to_data(img1)
    print(stringDATA)
    for x,data in enumerate(stringDATA.splitlines()):
        if x!=0 :
            wordData = data.split()
            print(wordData)
            if len(wordData) == 12:
                text=wordData[11]
                print(text)
                with open("textOutDATA.txt", 'a+') as file:
                    file.writelines(f"{text}\n")
                cv.rectangle(img, (int(wordData[6]), int(wordData[7])),(int(wordData[8])+int(wordData[6]),int(wordData[9])+int(wordData[7])), (100, 100, 0), thickness=2)
                cv.putText(img, f"{wordData[11]}", (int(wordData[6]), int(wordData[7])-8),cv.FONT_ITALIC, 1, (255, 0, 0), thickness=2)
                cv.putText(imageType, f"{wordData[11]}", (int(wordData[6]), int(wordData[7])),cv.FONT_ITALIC, 1, (0, 255, 0), thickness=3)
