...
def CannyScale(imgBlur):
    imgCanny=cv.Canny(imgBlur,130,130)
    return imgCanny
...
#text
def text(img):
    imageType=np.ones_like(img)
    imageType=imageType*100
    himg,wimg,l=img.shape
    img1 = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    stringBOX=pytesseract.image_to_boxes(img1)
    charBoxlist=stringBOX.split("\n")
    charBoxlist.remove('')
    print(charBoxlist)
    for char in charBoxlist:
        chardetails = char.split(" ")
        print(chardetails)
        if len(chardetails)>=5:
            print("dvsd")
            cv.rectangle(img,(int(chardetails[1]),himg-int(chardetails[2])),(int(chardetails[3]),himg-int(chardetails[4])),(100,100,0),thickness=2)
            cv.putText(img,f"{chardetails[0]}",(int(chardetails[1]),himg-int(chardetails[2])+25),cv.FONT_ITALIC,1,(255,0,0),thickness=2)
            cv.putText(imageType,f"{chardetails[0]}",(int(chardetails[1]),himg-int(chardetails[2])),cv.FONT_ITALIC,1,(0,255,0),thickness=2)
    return imageType
